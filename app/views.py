    ...
    if request.method == 'POST':
        ...
        if user:
            ...
            if bcrypt.checkpw(request.POST['password'].encode(),  usuario_login.password.encode()):
                ...
                request.session['usuario'] = { 'nombre':usuario_login.name , 'email':usuario_login.username,'role':usuario_login.role, 'role_nombre':usuario_login.get_role_display(), 'id':usuario_login.id}
                return redirect('/')
            else:
